...
1. **Idempotent no-op:** if the policy's end state already holds — for
   `merge` or `fast-forward`, `issue story get <storyId> merged` stdout is
   exactly `true`; for `pull-request`, `issue story get <storyId> prUrl`
   stdout is non-empty — do nothing and stop (success).
