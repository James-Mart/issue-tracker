...
describe("story worktree remove", () => {
  ...
  it("removes a clean worktree with --allow-active-run while a session is live", async () => {
    const workspace = initRepo();
    seedProject(workspace);
    writeStory("a");
    ...
    const result = await runIssueCli(
      ["story", "worktree", "remove", "a", "--allow-active-run"],
      { env: env() },
    );
    expect(result.status).toBe(0);
    expect(result.stdout.trim()).toBe(expectedPath);
  ...
  it("refuses --allow-active-run without --discard on a dirty checkout", async () => {
    const workspace = initRepo();
    seedProject(workspace);
    writeStory("a");
