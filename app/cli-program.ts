...
function createIssueProgram(run: Run): Command {
...
function captureStdio(): {
  ...
  const hookWrite = (chunks: string[]): NodeJS.WriteStream["write"] =>
  ...
  process.stdout.write = hookWrite(stdout);
  process.stderr.write = hookWrite(stderr);
  console.log = hookConsole(stdout);
  console.info = hookConsole(stdout);
  console.warn = hookConsole(stderr);
  console.error = hookConsole(stderr);
  return {
...
export async function runIssueCli(
  ...
  try {
    ...
    try {
      await createIssueProgram(run).parseAsync(args, { from: "user" });
    } catch (err) {
    ...
    return { ...stdio.output(), status };
  } catch (err) {
    console.error(err instanceof Error ? err.message : String(err));
    return { ...stdio.output(), status: 1 };
  } finally {
    stdio.restore();
    restoreStdin();
    restoreEnv();
