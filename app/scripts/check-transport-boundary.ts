...
// `new EventSource(...)` appears outside the transport module.
//
...
const TRANSPORT_MODULE = resolve(SRC_DIR, "lib/ws/transport.ts");
...
if (violations.length === 0) {
  console.log(
    "transport-boundary: OK — only the transport module constructs WebSocket/EventSource.",
  );
  process.exit(0);
}

console.error(
  `transport-boundary: ${violations.length} file(s) construct WebSocket or EventSource outside the transport module.\n` +
    "Components must subscribe via the shared transport client; only `src/lib/ws/transport.ts` may open a connection.\n",
);
